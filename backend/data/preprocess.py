...
class Preprocessor:
    def generate_data(self):
        ...
        df = df.drop(columns = ['id', 'is_bilingual'])  # Drop useless columnds
        ...
        for index, occupation in enumerate(df['occupation'].values):
            salary = salary_list[list(occupations).index(occupation)]
            salaries[index] = np.round(np.random.normal(salary, 0.02 * salary), decimals = 0)
    ...
    def create_occupation_salary_df():
        occupation_df = pd.read_csv('./occupations.csv')
